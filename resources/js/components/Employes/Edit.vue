...
const loadEmploye = async () => {
  ...
  try {
    ...
    form.value = {
      nom: employe.value.nom,
      prenom: employe.value.prenom,
      email: employe.value.email || '',
      telephone: employe.value.telephone || '',
      poste: employe.value.poste,
      salaire_mensuel: employe.value.salaire_mensuel,
      date_embauche: employe.value.date_embauche,
      statut: employe.value.statut,
      solde_conges_annuel: employe.value.solde_conges_annuel,
      equipe_id: employe.value.equipe_id
    };
  } catch (error) {
    console.error('Erreur lors du chargement:', error);
    errorMessage.value = 'Impossible de charger les données de l\'employé';
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    await employeService.update(route.params.id, form.value);

    // Rediriger vers la liste avec un message de succès
    router.push({
      name: 'employes',
      state: { success: `L'employé "${form.value.prenom} ${form.value.nom}" a été modifié avec succès` }
    });
  } catch (error) {
